...
def get_metadata(image_path):
    ...
    ) = get_calibration_matrices(tags, ifds)
...
def polynomial(current_image, polynomial_opcode):
    ...
    for i, idx in enumerate(idx2by2):
        ...
        planeTransform = (
            coefficient[3] * np.power(plane, 3)
            + coefficient[2] * np.power(plane, 2)
            + (coefficient[1]) * np.power(plane, 1)
            + coefficient[0]
        )
...
def performInterpolation(xyz_image, lut_table):
    ...
    # xyz_out = (
    # rgb2xyz[np.newaxis, np.newaxis, :, :] * rgbChanged[:, :, np.newaxis, :]
    # )
...
def performLookTable(xyz_image, lut):
    def interp(samples, lut_table):
        ...
        outInterpolate = scipy.interpolate.interpn(
            points=p, values=lut_table, xi=samples
        )
        print("outinterpolate", outInterpolate)
        samplesOriginal[:, :, 0] = samplesOriginal[
            :, :, 0
        ]  # + outInterpolate[:,:,0])
        samplesOriginal[:, :, 1] = np.clip(
            samplesOriginal[:, :, 1] * outInterpolate[:, :, 1], 0, 1
        )
        samplesOriginal[:, :, 2] = np.clip(
            samplesOriginal[:, :, 2] * outInterpolate[:, :, 2], 0, 1
        )
        print("samplesOriginal", samplesOriginal)
...
def transform_xyz_to_srgb(xyz_image):
    ...
    srgb_image = (
        xyz2srgb[np.newaxis, np.newaxis, :, :] * xyz_image[:, :, np.newaxis, :]
    )
...
def apply_tone_map(x):
    ...
    f = open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "acr3.txt"),
        "r",
    )
