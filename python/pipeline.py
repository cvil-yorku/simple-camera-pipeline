...
def run_pipeline_v2(
    image_or_path, params=None, metadata=None, fix_orient=True
):
    params_ = params.copy()
    ...
    if params_["input_stage"] == current_stage:
        ...
        current_image = normalize(
            current_image, metadata["black_level"], metadata["white_level"]
        )
    ...
    if params_["input_stage"] == current_stage:
        ...
        if gain_map_opcode is not None:
            ...
            current_image = lens_shading_correction(
                current_image,
                gain_map_opcode=gain_map_opcode,
    ...
    if params_["input_stage"] == current_stage:
        current_image = white_balance(
            current_image, metadata["as_shot_neutral"], metadata["cfa_pattern"]
        )
    ...
    if params_["input_stage"] == current_stage:
        ...
        if vignetting_opcode is not None:
            print("VIGNETTING")
            current_image = vignetting_correction(
                current_image, vignetting_opcode=vignetting_opcode
            )
    ...
    if params_["input_stage"] == current_stage:
        current_image = apply_color_space_transform(
            current_image,
            metadata["camera_calibration_1"],
            metadata["camera_calibration_2"],
            metadata["forward_matrix_1"],
            metadata["forward_matrix_2"],
    ...
    if params_["input_stage"] == current_stage:
        ...
        if hsv_lut is not None:
            current_image = performInterpolation(
                current_image, hsv_lut
            )  # hsv sat map

        if profile_lut is not None:
            current_image = performInterpolation(
                current_image, profile_lut
            )  # profile map
    ...
    raise ValueError(
        "Invalid input/output stage: input_stage = {}, output_stage = {}".format(
            params_["input_stage"], params_["output_stage"]
        )
    )
...
def run_pipeline(image_path, params):
    ...
    normalized_image = normalize(
        raw_image, metadata["black_level"], metadata["white_level"]
    )

    if params["output_stage"] == "normal":
        return normalized_image

    white_balanced_image = white_balance(
        normalized_image, metadata["as_shot_neutral"], metadata["cfa_pattern"]
    )

    if params["output_stage"] == "white_balance":
        return white_balanced_image

    demosaiced_image = demosaic(
    ...
    demosaiced_image = fix_orientation(
        demosaiced_image, metadata["orientation"]
    )

    if params["output_stage"] == "demosaic":
        return demosaiced_image

    xyz_image = apply_color_space_transform(
        demosaiced_image,
        metadata["color_corection_1"],
